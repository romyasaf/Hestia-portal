...
import { listAnnouncementsForOwnerUser } from "@/server/queries/announcements";

export default async function OwnerAnnouncementsPage() {
  const session = await requireSession();
  const items = await listAnnouncementsForOwnerUser(session.user.id);
    ...
    <div className="mx-auto max-w-3xl space-y-8 px-4 py-8">
      <header>
        ...
        <p className="mt-1 text-sm text-muted-foreground">
          Owner-wide and building-specific notices for properties you own.
        </p>
      </header>
