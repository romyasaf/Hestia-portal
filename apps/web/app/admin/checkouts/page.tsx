...
import { listCheckoutsForAdmin } from "@/server/queries/checkout";
import { cn } from "@/lib/utils";

export default async function AdminCheckoutsPage() {
  const rows = await listCheckoutsForAdmin();

  return (
    <div className="mx-auto max-w-6xl space-y-6 px-4 py-8">
      <header>
        <h1 className="text-2xl font-semibold tracking-tight">Checkouts</h1>
        <p className="mt-1 text-sm text-muted-foreground">Schedule visits and close checkout requests.</p>
      </header>

      {rows.length === 0 ? (
        <p className="text-sm text-muted-foreground">No checkout requests yet.</p>
      ) : (
            ...
            <tbody className="divide-y divide-border">
              {rows.map((r) => (
                <tr
                  ...
                  className={cn(
                    ...
                    (r.status === "requested" || r.status === "scheduled") &&
                      !isTerminalCheckoutStatus(r.status) &&
                ...
                >
                  ...
                  <td className="px-4 py-3">
                    <Link href={`/admin/checkouts/${r.id}`} className="font-medium text-primary hover:underline">
                      {r.tenantName}
                    </Link>
                  ...
                  <td className="px-4 py-3 text-muted-foreground">{r.preferredMoveOutDate ?? "—"}</td>
